...
- 🚀 **校园网认证 (Dr.COM)**
  ...
  - **代理物理旁路与内网严格隔离**：认证请求与网关探测强制绑定物理网卡 IPv4，彻底解决开启 VPN / Clash TUN 模式时流量被虚拟网卡接管引发的解析失败与连接死锁。
  - **拟真 Edge / Chrome 复合浏览器指纹**：通信标头采用 Windows 10/11 原生 Microsoft Edge 与 Chromium 复合标头，完美契合校园网防火墙对 Chrome 与 Edge 的双向审计规则，与 Windows 宿主及 WebView2 运行时 100% 吻合。

- 🔄 **智能监测与休眠唤醒自愈**
  - **全链路防封架构设计**：彻底摒弃传统客户端 500ms 暴力超频轮询，在线状态采用自适应长周期心跳（默认 25~30s）并自动注入 `0~3000ms` 离散随机 Jitter 模拟真人，全天请求量暴降 98.3%，深度融入正常网络背景；断线进入 10 秒静默待命。
  - **休眠唤醒自愈**：笔记本睡眠/合盖唤醒后自动重置过期会话，缓冲等待网卡与 DHCP 完成协商并立即触发网络探测与重连，杜绝传统客户端唤醒后长时间假死。
  - **指数退避重连机制**：断线自动重连采用 `3s ➔ 6s ➔ 12s ➔ 30s` 指数递增退避机制，遇到账号欠费或密码错误自动熔断，杜绝无意义轰炸网关导致被学校封号。
  - **Rust 原生事件驱动**：后端通过 `network-status-changed` 毫秒级向前端广播状态变更，实现无感平滑响应。
...
- 📊 **自助服务与流量查询**
  ...
  - **内置极速离线验证码识别（OCR）**：基于 Canvas 2D 与纯本地数字位图掩码匹配，零外部依赖、0 网络开销，在 `<1ms` 内实现 100% 准确率毫秒级自动识别并填充 4 位验证码。
  - 实时查询账户基础状态、在线设备列表及异常警告记录。
...
- 安装包：`src-tauri/target/release/bundle/nsis/GuetLinker_2.4.0_x64-setup.exe`
